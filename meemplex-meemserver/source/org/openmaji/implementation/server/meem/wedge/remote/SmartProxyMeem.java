...
public class SmartProxyMeem implements InvocationHandler, Serializable, WeakReferenceProvider {
	...
	private transient Set<Reference> lifeCycleClientReferences = null;
	private transient LifeCycleConnector lifeCycleConnector;
	private transient long leaseRetryTime;

	private transient Set<Reference> meemClientReferences = null;

	protected static Map<MeemPath, SmartProxyMeem> smartProxyMeemMap = new HashMap<MeemPath, SmartProxyMeem>();
	...
	public Object readResolve() {
		// If not deserialized in a Meemserver, return this without creating
		// leases etc
		...
		synchronized (smartProxyMeemMap) {
			SmartProxyMeem spm = smartProxyMeemMap.get(meemPath);
			if (spm != null) {
				if (replaceWithNewest) {
	...
	public Meem getSmartProxyMeem() {
		if (remoteMeem != null && smartProxyMeem == null) {
			try {
				smartProxyMeem = (Meem) Proxy.newProxyInstance(getClass().getClassLoader(), new Class[] { org.openmaji.meem.Meem.class }, this);
			}
	 ...
	 * 
	 * @param proxy
	 *            Proxy instance that the method was invoked upon
	 * @param method
	 *            Corresponds to the interface method invoked on the proxy
	 * @param args
	 *            Array of objects containing the values of the arguments
	 * @return Value to return from the method invocation on the proxy
	 * @exception Throwable
	 *                Thrown from the actual method invocation
	 */
	public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
				...
				if (facetIdentifier.equals("meemClientFacet")) {
					if (reference.getFilter() != null && reference.getFilter() instanceof FacetFilter) {
						...
						if (filter.match("lifeCycleClient", LifeCycleClient.class, Direction.OUTBOUND)) {
							// if its an exact match, then we don't need to hand
							// it through to the real meem
					...
					if (reference.getFilter() == null) {
						...
						// then we always send it across the wire. We also need
						// to keep it though
		...
		try {
			...
			// new ReflectionInvocation("meem", method, args,
			// RequestTracker.getRequestStack()).fail();
			// System.err.println("SmartProxyMeem.invoke(): " + lease + " : " +
			// meemPath + " : " + t.getMessage());
		}
	 ...
	 */
	public WeakReference<SmartProxyMeem> obtainWeakReference() {
		...
		if (weakReference == null) {
			...
			// System.err.println("notify: " + meemPath + " : " +
	...
	private synchronized Set<Reference> getLifeCycleClientReferences() {
		if (lifeCycleClientReferences == null) {
			lifeCycleClientReferences = new HashSet<Reference>();
		}

		return lifeCycleClientReferences;
	}

	private synchronized Set<Reference> getMeemClientReferences() {
		if (meemClientReferences == null) {
			meemClientReferences = new HashSet<Reference>();
		}
	...
	private void notifyLastTransition(LifeCycleTransition transition) {
		Set<Reference> references = getLifeCycleClientReferences();
	...
	private void attemptLeaseRenewal() {
		// System.err.println("attemptLeaseRenewal(): " + meemPath + " : "+
		// leaseRetryTime);
	...
	private final class RenewLeaseTimerTask extends TimerTask {
		...
		public void run() {
			...
			weakReference = new WeakReference<SmartProxyMeem>(SmartProxyMeem.this);
			...
			renewingLease = false;
			// System.err.println("SmartProxyMeem.connectLease(): " + meemPath +
			// " : "+ lease + " : " + weakReference);
	...
	private synchronized void addMeemClientReference(Reference reference, boolean automaticRemove) {
		...
		if (target instanceof MeemClient) {
			if (reference.isContentRequired()) {
				...
				for (Reference ref : getLifeCycleClientReferences()) {
	...
	private void notifyReferenceChange(Reference reference, boolean added) {
		for (Reference ref : getMeemClientReferences()) {
	...
	private final class LifeCycleConnector {
		...
		public void connect() {
			disposed = false;
			// logger.log(Level.WARNING, "connect(): " + meemPath + " : " +
			// lifeCycleClient + " : " + this);

			Serializable[] args = new Serializable[] { reference, Boolean.FALSE };

			try {
				// if we have just been unmarshalled, there will not be a
				// request stack, so we set one in the IC
		...
		public void disconnect() {
			...
			// only remove the reference if the lease is still valid. If it is
			// invalid, then there is no connection. The SPMRemoteReference will
					...
					if (!transition.getCurrentState().equals(actualCurrentState)) {
						...
						if (!expectedCurrentState.equals(actualCurrentState)) {
							...
							do {
								...
								LifeCycleTransition nextTransition = new LifeCycleTransition((LifeCycleState) LifeCycleState.STATES.get(actualIndex),
