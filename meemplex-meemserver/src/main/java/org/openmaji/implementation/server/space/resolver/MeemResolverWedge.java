...
public class MeemResolverWedge implements Wedge, MeemDefinitionProvider {
	...
	public MeemClient meemReferenceClientConduit = new MeemClient() {
		public void referenceAdded(Reference<?> reference) {
			...
			if (target instanceof MeemResolverClient) {
				...
				MeemPath meemPath = ((ExactMatchFilter<MeemPath>) reference.getFilter()).getTemplate();
	...
	// TODO why is this here when there is a continuous monitor created on
	// referenceAdded for the MeemResolverClient facet?
	public final AsyncContentProvider<MeemResolverClient> meemResolverClientProvider = new AsyncContentProvider<MeemResolverClient>() {
		public void asyncSendContent(MeemResolverClient client, Filter filter, ContentClient contentClient) {
			if (filter instanceof ExactMatchFilter) {
				MeemPath meemPath = ((ExactMatchFilter<MeemPath>) filter).getTemplate();
				if (meemPath == null) {
	...
	/**
	 * @see org.openmaji.space.resolver.MeemResolver#resolveMeemPath(org.openmaji.meem.MeemPath,
	 *      org.openmaji.space.resolver.MeemResolverClient)
	 */
	private void resolveMeem(MeemPath meemPath, final MeemResolverClient client, final ContentClient contentClient) {
		if (meemPath.isDefinitive()) {
			...
			// if (meem == null) {
			// contentClient.contentFailed("MeemPathResolver cannot find HyperSpace while trying to resolve "
			// + meemPath);
			// }
			// else {
			// new ResolvedMeemPathCategoryCallback(meemPath, client,
			// contentClient).proceed(meem);
			// }
		}
		else {
	...
	private synchronized void startMonitoring(MeemPath meemPath) {
		if (meemPath.isDefinitive()) {
			...
			// MeemPath currentMeemPath =
			// MeemPath.spi.create(meemPath.getSpace(), "");
			// String firstPath = tok.nextToken();
			//
			// createCategoryMonitor(meem, currentMeemPath, firstPath,
			// meemPath);
			// }
			// else {
	...
	public synchronized void pathResolved(MeemPath searchMeemPath, MeemPath resolvedMeemPath) {
		if (Common.TRACE_ENABLED && Common.TRACE_MEEMPATHRESOLVER) {
			logger.log(LOG_LEVEL, "pathResolved() searchMeemPath: " + searchMeemPath + " resolvedMeemPath: " + resolvedMeemPath);
		}
	...
	/**
	 * @param newEntryPath
	 *            non-storage path of resolved entry (eg: hyperspace:/a/b/c);
	 * @param path
	 *            storage path of resolved entry (eg: meemstore:/uid)
	 * @param meemPaths
	 *            Vector of MeemPaths that are to be watched
	 */
	public synchronized void handleEntryResolved(MeemPath newEntryPath, MeemPath path, List<MeemPath> meemPaths) {
		if (Common.TRACE_ENABLED && Common.TRACE_MEEMPATHRESOLVER) {
		...
		for (MeemPath meemPath : meemPaths) {
			if (meemPath.equals(newEntryPath)) {
				...
				synchronized (monitorDetails) {
					...
					if (monitors == null) {
						monitors = new ArrayList<MonitorDetail>();
	...
	public synchronized void handleMeemRegistered(MeemPath meemPath, Meem meem) {
		...
		if (monitors != null) {
			facetClientConduit.hasA(meem, "categoryClient", CategoryClient.class, Direction.OUTBOUND, new MeemRegisteredCategoryCallback(meem, monitors));
		}
		else {
			ResolvedDetail resolvedDetail = (ResolvedDetail) resolvedDetails.get(meemPath);
			if (resolvedDetail != null) {
				pathResolvedMeemRegistered(meem, resolvedDetail.getSearchMeemPath(), resolvedDetail.getResolvedMeemPath());
			}
		}
	}
	...
	public synchronized void handleEntryRemoved(final MeemPath removedEntryPath, List<MeemPath> meemPaths) {
		...
		// we need to trash all the CategoryWatchers with meempaths that start
		// with this meempath
	 ...
	 * 
	 * @param category
	 *            Category Meem to watch
	 * @param categoryPath
	 *            Non-storage path to the category
	 * @param entryName
	 *            entry name to watch for
	 * @param fullMeemPath
	 *            full meempath of the meem that is ultimately being watched for
	 */
	private void createCategoryMonitor(Meem categoryMeem, MeemPath categoryPath, String entryName, MeemPath fullMeemPath) {
		...
		if (pair == null) {
			...
			CategoryClient categoryClient = meemCore.getTargetFor(categoryMonitor, CategoryClient.class);
			Reference<CategoryClient> reference = Reference.spi.create("categoryClient", categoryClient, true);
	...
	public final class ResolvedMeemPathCategoryCallback implements CategoryClient, ContentClient {
		...
		/**
		 * @see org.openmaji.system.space.CategoryClient#entryRenamed(org.openmaji.system.space.CategoryEntry,
		 *      org.openmaji.system.space.CategoryEntry)
		 */
		...
		public void proceed(Meem meem) {
			...
			if (tok.hasMoreTokens()) {
				...
				Reference<Facet> categoryClientReference = Reference.spi.create("categoryClient", categoryClientTarget, true, filter);
	...
	private final class RegistryLookup implements MeemRegistryClient, ContentClient {
		...
		public RegistryLookup(MeemPath meemPath, MeemResolverClient meemResolverClient, ContentClient contentClient) {
			...
			Reference<MeemRegistryClient> reference = Reference.spi.create("meemRegistryClient", meemCore.getLimitedTargetFor(this, MeemRegistryClient.class), true, ExactMatchFilter.create(meemPath));
	...
	private final class MeemRegisteredCategoryCallback implements FacetClientCallback {
		...
		public void facetExists(boolean available) {
			if (!available) {
				// not a category. bad.
				throw new RuntimeException("MeemResolver expecting a Category got something else: " + meem.getMeemPath());
			}

			if (Common.TRACE_ENABLED && Common.TRACE_MEEMPATHRESOLVER) {
				logger.log(LOG_LEVEL, "handleMeemRegistered() " + meem.getMeemPath() + " : monitors: " + monitors);
			}

			for (MonitorDetail monitorDetail : monitors) {
				createCategoryMonitor(meem, monitorDetail.getCategoryPath(), monitorDetail.getEntryName(), monitorDetail.getFullMeemPath());
			}
		}
	}
