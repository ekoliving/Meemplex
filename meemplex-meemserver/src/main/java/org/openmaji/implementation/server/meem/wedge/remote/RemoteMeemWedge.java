...
public class RemoteMeemWedge implements Wedge, RemoteMeem {
	...
	public final ContentProvider<RemoteMeemClient> remoteMeemClientFacetProvider = new ContentProvider<RemoteMeemClient>() {
		public synchronized void sendContent(RemoteMeemClient target, Filter filter) {
			// [TODO] MUST REPLACE this WITH THE MeemInvocationTarget PROXY FOR
			// THIS WEDGE
			doRemoteMeemChanged(target);
		}
	};

	private FacetItem[] facetItems = null;

	private void doRemoteMeemChanged(RemoteMeemClient target) {

		ArrayList<FacetItem> items = new ArrayList<FacetItem>();

		if (facetItems == null) {
			if (meemCore.isA(HyperSpace.class)) {
				items.add(new FacetItem("hyperSpace", HyperSpace.class.getName(), Direction.INBOUND));
			}
			if (meemCore.isA(MeemStore.class) && !meemCore.isA(MeemStoreProxy.class) && meemCore.getSelf().equals(EssentialMeemHelper.getEssentialMeem(MeemStore.spi.getIdentifier()))) {
				items.add(new FacetItem("meemStore", MeemStore.class.getName(), Direction.INBOUND));
			}
			if (meemCore.isA(JiniMeemRegistry.class)) {
				items.add(new FacetItem("jiniMeemRegistry", JiniMeemRegistry.class.getName(), Direction.INBOUND));
			}
			if (meemCore.isA(MeemkitLifeCycleManager.class)) {
				items.add(new FacetItem(MeemkitLifeCycleManager.spi.getIdentifier(), MeemkitLifeCycleManager.class.getName(), Direction.INBOUND));
			}
			if (meemCore.isA(MeemServer.class)) {
				items.add(new FacetItem("meemServer", MeemServer.class.getName(), Direction.INBOUND));
			}
			items.add(new FacetItem("meem", Meem.class.getName(), Direction.INBOUND));
		}
	 ...
	 * Remote Invocation.
	 */
	public void majikInvocation(String facetIdentifier, String methodName, Class<?>[] argsClasses, Serializable[] args, Serializable request) throws RemoteException {
		...
		try {
			...
			if (object instanceof MeemInvocationTarget) {
				...
				return;
				// throw new RemoteException(
				// "majikInvocation(): No such facetIdentifier=" +
				// facetIdentifier
				// );
			}
			...
			if (implementation == null) {
				// System.err.println("Call to facet [" + facetIdentifier + "."
				// + methodName + "] on revoked proxy");
				// throw new
				// RemoteException("majikInvocation(): Proxy has been revoked");
			}
			else {
				...
				// System.err.println("Remote: rs = " + request + " : " +
				// getDescription(method, args));
			...
			throw new RemoteException("majikInvocation(): No such method=" + methodName);
		}
