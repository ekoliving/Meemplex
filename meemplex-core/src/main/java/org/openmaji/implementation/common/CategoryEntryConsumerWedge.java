...
public class CategoryEntryConsumerWedge implements Wedge, CategoryEntryConsumer {
	...
	public final ContentProvider<CategoryEntryConsumer> entryConsumerClientProvider = new ContentProvider<CategoryEntryConsumer>() {
		public synchronized void sendContent(CategoryEntryConsumer target, Filter filter) {
			if (DebugFlag.TRACE)
	...
	/**
	 * CategoryEntry maintained by this Wedge and persisted by the Maji
	 * framework
	 */
	...
	/**
	 * The conduit through which categoryentry changes are received other Wedges
	 * in the Meem.
	 */
	...
	public synchronized void entry(CategoryEntry entry) {
		if (DebugFlag.TRACE)
			logger.log(Level.FINE, "entry() - invoked on in-bound facet");
	...
	class EntryConsumerStateConduit implements CategoryEntryConsumer {
		...
		public synchronized void entry(CategoryEntry newEntry) {
			if (DebugFlag.TRACE)
				logger.log(Level.FINE, "entry() - invoked on CategoryEntryConsumerStateConduit");
			entry = newEntry;
