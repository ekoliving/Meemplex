...
/**
 * UnavailableMeemController
 * 
 * Provides actions to perform on an UnavailableMeemNode.
 * 
 * @author Warren Bloomer
 */
public class UnavailableMeemController extends Controller {
	 ...
	 * @param node
	 */
	public UnavailableMeemController(UnavailableMeemNode node) {
		super(node);
	}

	protected UnavailableMeemNode getMeemNode() {
		return (UnavailableMeemNode) getNode();
	}
	...
	protected void createActions() {
		...
		removeAction.setImageDescriptor(WorkbenchImages.getImageDescriptor(ISharedImages.IMG_TOOL_DELETE));
...
//	}
	...
	protected Collection getCategoryEntryNames(CategoryProxy category) {
		...
		for (int i = 0; i < entries.length; i++) {
			entryNames.add(entries[i].getName());
		}
		return entryNames;
	}
